(function () {
  ...
  try {
    ...
    navigator.clipboard.writeText = function(text) {
      ...
      var capturing = document.documentElement.getAttribute('data-max-export-capturing') === '1';
  ...
  function _captureUrlSlug() {
    ...
    var slugM = path.match(/^\/([a-zA-Z][a-zA-Z0-9_]{2,30})(?:$|[/?#])/);
    var idM = path.match(/^\/(-?\d+)/);
    if (slugM && !idM) {
    ...
    if (idM) {
      if (_pendingSlug) {
        try {
          var map = JSON.parse(sessionStorage.getItem('_maxExportSlugMap') || '{}');
  ...
  history.pushState = function() { var r = _origPushState.apply(this, arguments); _captureUrlSlug(); return r; };
  history.replaceState = function() { var r = _origReplaceState.apply(this, arguments); _captureUrlSlug(); return r; };
